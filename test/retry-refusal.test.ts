...
describe("retry-after-refusal extension wiring", () => {
    it("announces the retry, switches to Opus, resends the last user prompt, and restores the previous model", async () => {
        ...
        const fableModel = { provider: "anthropic", id: "claude-fable-5", name: "Claude Fable 5" }
        const opusModel = { provider: "anthropic", id: "claude-opus-4-8", name: "Claude Opus 4.8" }
        const pi = {
        ...
        const ctx = {
            ...
            modelRegistry: {
                find(provider: string, id: string) {
                    return provider === "anthropic" && id === "claude-opus-4-8" ? opusModel : undefined
                },
            },
            ui: {
        ...
        await handlers.message_end![0]!({ message: { role: "user", content: "Explain a risky topic" } }, ctx)
        ...
        await handlers.agent_end![0]!({ messages: [{ role: "assistant", model: "claude-fable-5" }] }, ctx)
        expect(selectedModels).toEqual([opusModel])

        await handlers.agent_end![0]!({ messages: [{ role: "assistant", model: "claude-opus-4-8" }] }, ctx)
    ...
    it("does not claim a retry when Opus cannot be selected", async () => {
        ...
        const ctx = {
            model: { provider: "anthropic", id: "claude-fable-5", name: "Claude Fable 5" },
            modelRegistry: {
                find(_provider: string, id: string) {
                    if (id === "claude-fable-5") return { provider: "anthropic", id, name: "Claude Fable 5" }
                    if (id === "claude-opus-4-8") return { provider: "anthropic", id, name: "Claude Opus 4.8" }
                    return undefined
                },
            },
            ui: {
        ...
        expect(notifications).toEqual([
            {
                message: "Claude Fable 5 refusal detected, but Claude Opus 4.8 could not be selected.",
                kind: "error",
            },
        ])
        expect(sentUserMessages).toEqual([])
