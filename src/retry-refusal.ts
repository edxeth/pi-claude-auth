...
function isRefusalError(message: unknown): boolean {
	...
	return /refus|classifier|safety|usage policy|violative|refusals-and-fallback/i.test(
		message,
	);
}

export function shouldRetryWithOpus(
...
function getRetryableUserContent(
	content: unknown,
): RetryUserContent | undefined {
...
export function createRetryAfterRefusalState() {
	...
	return {
		...
		consumeRetry(
			...
			if (!lastUserContent || !lastUserKey || retriedUserKey === lastUserKey)
				return undefined;
...
interface RetryAfterRefusalContext {
	model?: unknown;
	modelRegistry: {
...
export function registerRetryAfterRefusal(pi: RetryAfterRefusalPi): void {
	...
	pi.on("message_end", async (event, ctx) => {
		const message = event.message as RetryableUserMessage &
			AssistantRefusalCandidate;
